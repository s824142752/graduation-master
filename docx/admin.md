...
用户管理服务进一步分成：
* 用户管理服务，见AdminUserController类
* 用户足迹服务，见AdminFootprintController类
...
权限控制在数据库层面涉及到三个表`gradu_admin`, `gradu_role`和`gradu_permission`：
* gradu_admin表中存在roleId字段，保存角色ID数组；
* gradu_role表记录角色名称和角色介绍；
* gradu_permission表记录角色所用于的权限值。
...
后端权限基于shiro来实现，相关代码见gradu-admin-api模块。
...
> 这里就带来了实现方面的问题:
> 一些开源项目在数据库设计一个permission表，记录系统所有权限，但是4.1.6.1.1节讨论了这种方式的局限性。
...
gradu-admin-web模块的代码基于[vue-element-admin](https://github.com/PanJiaChen/vue-element-admin)
