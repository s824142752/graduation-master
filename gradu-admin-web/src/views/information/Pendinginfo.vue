<template>
  ...
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <!--      <el-table-column align="center" width="80" label="序号" prop="id" fixed="left" sortable/>-->
      ...
      <el-table-column align="center" label="操作" width="210" class-name="small-padding fixed-width" fixed="right">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" @click="handleDetail(scope.row)">处理</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <!-- 用户编辑对话框 -->
    <el-dialog :visible.sync="uploadDialogVisible" :data="list" title="信息处理">
      <div class="dialogDiv">
        <el-form ref="Detail" :model="Detail" status-icon label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
          ...
          <!--          >-->
          <!--            <img v-if="Detail.img1" :src="Detail.img1" class="avatar">-->
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="handleToRefuse">拒绝</el-button>
        <el-button type="primary" @click="handleToAgree">同意</el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  methods: {
    getList() {
      ...
      listVoInfo(0).then(response => {
        this.list = []
        if (response.data.data) {
          // console.log(response.data.data.list[0].graduUser.nickname);
    ...
    * */
    /*uploadAvatar: function(response) {
      this.Detail.img1 = response.data.url  //单传
    },*/
    ...
    handlePictureUrl(response, file, fileList) {
      if (response.errno === 0) {
        ...
        console.log(this.Detail.img2.push(response.data.url))
      }
    },
    /*
    * 删除图片，可以选择之后直接使用Delete键
    * */
