...
public class WxSearchController {
    ...
    @Autowired
    private GraduKeywordService keywordsService;
    @Autowired
    ...
    public Object index(@LoginUser Integer userId) {
        //取出输入框默认的关键词
        GraduKeyword defaultKeyword = keywordsService.queryDefault();
        //取出热闹关键词
        List<GraduKeyword> hotKeywordList = keywordsService.queryHots();

        List<GraduSearchHistory> historyList = null;
        if (userId != null) {
            //取出用户历史关键字
    ...
    public Object helper(@NotEmpty String keyword,
        ...
        List<GraduKeyword> keywordsList = keywordsService.queryByKeyword(keyword, page, limit);
        String[] keys = new String[keywordsList.size()];
        int index = 0;
        for (GraduKeyword key : keywordsList) {
